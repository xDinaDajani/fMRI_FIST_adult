...
#### Collect RT values and put them on 0 to 5 scale
  ...
  #histogram of RT values
  hist(fmriRT.vector$Med_RT_Run1, main = "fMRI Run 1 Flex Trials Histogram")
  hist(fmriRT.vector$Med_RT_Run2, main = "fMRI Run 2 Flex Trials Histogram")
  hist(fmriRT.vector$Med_RT_Run3, main = "fMRI Run 3 Flex Trials Histogram")
  hist(fmriRT.vector$Med_RT_Run4, main = "fMRI Run 4 Flex Trials Histogram")
...
### ADD rescaled accuracy to rescaled RT
  ...
  accRT_DV_Run1 <-rowSums(rescaled_both[,c("fMRIRun1Acc_rescaled", "Med_RT_Run1_rescaled")])
  accRT_DV_Run2 <-rowSums(rescaled_both[,c("fMRIRun2Acc_rescaled", "Med_RT_Run2_rescaled")])
  accRT_DV_Run3 <-rowSums(rescaled_both[,c("fMRIRun3Acc_rescaled", "Med_RT_Run3_rescaled")])
  accRT_DV_Run4 <-rowSums(rescaled_both[,c("fMRIRun4Acc_rescaled", "Med_RT_Run4_rescaled")])
  
  fMRI_accRT_DV <-cbind(accRT_DV_Run1, accRT_DV_Run2, accRT_DV_Run3, accRT_DV_Run4)
  
  #Take a look at variability of this accRT DV
  describe(fMRI_accRT_DV)
  accv2.dat <- cbind(acc.dat, fMRI_accRT_DV)  
...
##======Write out new DV to CSV file====================================
  ...
  write.csv(accv2.dat, "Acc_data_fMRI_task_newDV_final4.23.18.csv", row.names=FALSE)

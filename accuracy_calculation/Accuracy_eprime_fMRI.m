...
% run number
run = regexp(content{1}, 'Experiment:', 'match'); 
runi = find(~cellfun('isempty', run));
runall = content{1}{runi(1)};
runnum = regexp(runall, 'Run\d', 'match');
disp(sprintf('The run number is %s', runnum{1}));
fclose(fileID); 
